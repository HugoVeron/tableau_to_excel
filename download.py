...
class Tableau_Server(object):
    ...
    def download_view_csv(self,wb_name, view_name = None):
        ...
        if 'Measure Values' not in df.columns.values:
            df.to_csv('{}.csv'.format(view_item.name), index = False, sep = "|")
            return
...
def download(workbook_str, sheet_str):
    username = ''
    password = ''
    site_id = ''
    url = ''
    ...
    # download a workbook
    #ts.download_wb('KPI_P0_4G')
    
    # download a view csv
    ts.download_view_csv(workbook_str,sheet_str)
